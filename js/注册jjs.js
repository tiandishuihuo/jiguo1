$(function() {
    $('.submit').click(function() {
        ...
        if ($('.repwd').val() != $('.pwd').val()) {
            ...
            $.ajax({
                url: "http://192.168.31.110:3000/users/register", //后台接口地址
                data: {
                    "phone": str,
                    "code": str1,
                    "username": str2,
...
window.onload = function () {
    ...
    var phone = document.getElementsByClassName('phone')[0],
        ...
        repwd = document.getElementsByClassName('repwd')[0];
    ...
    submit.addEventListener('click', function () {
        // 创建ajax对象
        var index1_a = new XMLHttpRequest() || new ActiveXObject("Microsoft.XMLHTTP");
        var data = `phone=${phone.value}&username=${username.value}&password=${pwd.value}`;
        // 链接url地址
        index1_a.open("POST", 'http://192.168.31.110:3000/users/register', true);
        // 如果需要像 HTML 表单那样 POST 数据，请使用 setRequestHeader() 
        // 来添加 HTTP 头。然后在 send() 方法中规定您希望发送的数据：
        index1_a.setRequestHeader('content-type', 'application/x-www-form-urlencoded');
        // 发送数据 
        index1_a.send(data);
        // 接收发送的返回值
        index1_a.onreadystatechange = function () {
            if (index1_a.readyState == 4) {
                if (index1_a.status >= 200 || index1_a.status < 300) {
                    console.log();
                    index_res = JSON.parse(index1_a.responseText);
    ...
    regin.addEventListener('click', function () {
        ...
        // 链接url地址
        index1_a.open("POST", 'http://192.168.31.110:3000/users/login', true);
        // 如果需要像 HTML 表单那样 POST 数据，请使用 setRequestHeader() 
        // 来添加 HTTP 头。然后在 send() 方法中规定您希望发送的数据：
        // index1_a.setRequestHeader('content-type', 'application/x-www-form-urlencoded');
        // 发送数据 
